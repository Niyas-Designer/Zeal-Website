...
function FloatingContactForm() {
  ...
  useEffect(() => {
    ...
    const hasShownThisSession =
      localStorage.getItem(SESSION_POPUP_KEY) === sessionId
  ...
  async function handleSubmit(event) {
    ...
    if (
      ...
      console.error('EmailJS environment variables are missing.', {
        ...
        publicKeyExists: Boolean(emailConfig.publicKey),
      })

      setSubmitState({
        type: 'error',
        message:
          'EmailJS is not configured yet. Please check the environment variables.',
      })
    ...
    try {
      ...
      const errorMessage =
        error?.text ||
        error?.message ||
        'Unknown EmailJS error. Please check the browser console.'
      ...
      <div className="pointer-events-none fixed bottom-5 right-5 z-50 flex w-[calc(100vw-2.5rem)] max-w-sm flex-col items-end gap-3 sm:bottom-6 sm:right-6">
          ...
          <form className="mt-5 space-y-4" onSubmit={handleSubmit}>
            <div>
              <label
                ...
                className="mb-2 block text-sm font-medium text-[#222]"
              >
                Name
              </label>
              ...
              {errors.name ? (
                <p className="mt-2 text-xs text-[#d84e4e]">
                  {errors.name}
                </p>
              ) : null}
            </div>
        ...
        <button
          ...
          className={`pointer-events-auto inline-flex h-14 w-14 items-center justify-center rounded-full bg-[#ff0c00] text-sm font-medium uppercase tracking-[0.16em] text-white shadow-[0_18px_42px_rgba(15,23,42,0.25)] transition duration-300 ease-out hover:-translate-y-0.5 hover:bg-[#ff0c11] sm:h-auto sm:w-auto sm:gap-3 sm:px-5 sm:py-3.5 ${
            isOpen
              ? ''
              : 'animate-[contact-float_4.2s_ease-in-out_infinite]'
          }`}
